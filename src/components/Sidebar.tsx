...
const Sidebar: React.FC<SidebarProps> = ({ articles, selectedCountry, onClose }) => {
  ...
  return (
    <div 
      className={`cyber-border-red fixed top-0 h-screen bg-cyber-black/90 backdrop-blur-md z-50 transition-all duration-300 transform shadow-lg
        ${isMobile ? 'right-0 w-full md:w-96 bottom-0 h-[50vh]' : 'right-0 w-full md:w-96 top-0 h-screen'}`}
      style={{ 
        top: isMobile ? '50%' : 0,
